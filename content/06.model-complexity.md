...
Overfit models tend to rely on variables that are unique to the training data (for example, the calibration of the instrument that was used to generate the training data).
When models are overfit, they are not generalizable to new data (e.g., data generated on the same instrument that has been recalibrated). [@doi:10.1073/pnas.1900654116]
...
Overfit models generate large errors when the model tries to predict outcome values for new evaluation data.
Regularization is a collection of methods (generally regression methods) that can help reduce these prediction errors by various approaches. 
...
In this study, applying LASSO regularization helped reduce the number of common variants selected to be included as features in the final analysis facilitating generation of a simpler model while reducing error in the association between common and rare variants with a specific trait. 
