...
<!-- TODO: Explain what is class imbalance --->
...
First, they assume the presence of equal number of samples for all classes in a dataset (i.e. no class imbalance). 
...
<!-- Previous content --->
Translating machine learning findings into testable hypotheses requires the ML models to be both stable (i.e. the same predicted features should surface from the data if the model is run multiple times), and simple (to guard against misinterpretation), while still showing high accuracy in presence of unseen test data. 
