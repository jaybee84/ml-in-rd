...
A lack of samples gives rise to the “curse of dimensionality” (i.e., few samples but many features), which can contribute to the poor performance of models [@doi:10.1038/nrc2294] (Figure {@fig:1}a).
...
In the first approach, appropriate training, evaluation, and held-out validation sets could be constructed by combining multiple small individual rare disease cohorts (Fig.1b, Box 1).
In the authors' experience, this is often a neccessity in case of rare diseases.
...
Without careful selection of aggregation methods, one may introduce technical variability into the combined dataset which can negatively impact the ML model's ability to learn or detect meaningful signal.
