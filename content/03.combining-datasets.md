...
![Combining datasets to increase data for training machine learning models. a) Appropriate methods are required to combine smaller datasets into a composite dataset without loss of information: The left panel shows multiple small rare disease datasets that need to be combined to form a dataset of higher sample size. The color of the samples suggest class or groups present in the datasets. The shape correlates to the identity of the datasets. The middle panel shows methods that may be used to combine the datasets. The right panel shows principle component analysis of the combined datasets to verify proper integration without loss of information. b) Appropriately prepared composite datasets can be used to make training, evaluation, and validation datasets for machine learning: Left panel shows the division of the composite dataset into training dataset and a held-out validation dataset (top). The training set is further divided into training and evaluation datasets for k-fold cross-validation. The panel on the right shows the class composition of the training, evaluation, and held-out validation datasets](content/images/figures/pdfs/figure-1-combining-datasets.png){#fig:1}
